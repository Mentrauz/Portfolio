...
const projects = [
  {
    ...
    // imageUrl: "/tms.png",
    demoUrl: "https://payroll.soumyasingh.site/",
    githubUrl: "https://github.com/Mentrauz/OpenRoll",
    featured: false,
    completed: "September 2024 - December 2025",
    teamSize: 1,
    difficulty: 5,
    inProgress: true,
    achievements: [
  ...
  return (
    <SectionContainer id="projects" className="relative">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90svh] overflow-hidden p-0 w-[calc(100vw-1rem)] sm:w-[95vw] bg-background/95 backdrop-blur-sm border-border/50 shadow-xl [&>button]:hidden">
          {selectedProject && (
            <div className="flex flex-col h-full max-h-[90svh]">
              ...
              <Tabs defaultValue="overview" className="flex-1 overflow-hidden">
                <div className="px-4 sm:px-6 border-b border-border/20 overflow-x-auto">
                  <TabsList className="justify-start rounded-none bg-transparent h-10 w-max">
                ...
                <div
                  ref={scrollRef}
                  data-lenis-prevent
                  className="p-4 sm:p-6 overflow-y-auto max-h-[calc(90svh-220px)] sm:max-h-[calc(90svh-200px)] overscroll-contain touch-pan-y custom-scrollbar"
    ...
    >
      <CardContent className="p-6 bg-card/50">
        ...
        <h3 className="text-base sm:text-lg font-semibold mb-2 text-foreground flex items-start gap-2 group-hover:text-primary transition-colors">
          {project.title}
          <ChevronIcon className="h-4 w-4 shrink-0 text-muted-foreground group-hover:text-foreground transition-colors" />
        </h3>
        ...
        <div className="flex flex-col gap-2 text-xs text-muted-foreground mt-auto sm:flex-row sm:justify-between sm:items-center">
          <div className="flex min-w-0 items-center gap-1">
            <Calendar className="h-3 w-3" />
            <span className="break-words">{project.completed}</span>
          </div>
