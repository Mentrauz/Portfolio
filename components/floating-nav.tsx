...
export default function FloatingNav() {
  ...
  const handleItemClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    if (element) {
      if (lenis) {
        lenis.scrollTo(element as HTMLElement, { offset: 0, lerp: 0.05 })
      } else {
    ...
    <>
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            exit={{ y: 100, opacity: 0 }}
            transition={{ duration: 0.3 }}
            className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] right-4 z-50 md:hidden"
          >
            <Button
              size="icon"
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="bg-background/95 backdrop-blur-lg rounded-lg border shadow-lg p-2 flex flex-col gap-1 w-48 max-w-[calc(100vw-2rem)]">
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
      ...
      <AnimatePresence>
        {showBackToTop && (
          <motion.div
            ...
            exit={{ y: 20, opacity: 0 }}
            transition={{ duration: 0.2 }}
            className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-4 z-50"
          >
            <Button
              size="icon"
              variant="outline"
              className="h-10 w-10 rounded-full shadow-md bg-background/80 backdrop-blur-sm hover:bg-accent"
              onClick={() => { if (lenis) { lenis.scrollTo(0, { lerp: 0.05 }) } else { window.scrollTo({ top: 0, behavior: "smooth" }) } }}
              title="Back to top"
            >
